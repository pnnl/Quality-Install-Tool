<Tabs>
  <Tab eventKey="pre-installation" title="Pre-Installation">
    ...
    <Select label="Do the existing ducts run primarily through conditioned or unconditioned space?" options={["Conditioned","Unconditioned","Don’t know"]} path="existing_duct_condition" />
    ...
    <Radio label="What is the total duct leakage before air sealing?" options={['CFM at 50Pa', 'Don\'t know',]} path="existing_duct_leakage_rate_radio" />
    <ShowOrHide visible={props.data?.existing_duct_leakage_rate_radio === 'CFM at 50Pa'}> 
    <NumberInput label="CFM at 50Pa" min={0} path="duct_leakage_cfm50"  />
    </ShowOrHide>
  ...
  <Tab eventKey="post_installation"  title="Post-Installation">
    ...
    <Radio label="What is the total duct leakage after air sealing?" options={['CFM at 50Pa', 'Don\'t know',]} path="postinstall_duct_leakage_rate" />
 ...
 <Tab eventKey="report"  title="Report">
    ...
    <p> Existing ducts run primarily through conditioned or unconditioned space:   <strong> { props.data.existing_duct_condition ? props.data.existing_duct_condition : null } </strong> </p>
    ...
    <Photo id="existing_static_pressure_test_setup_photo" label="Existing condition total static pressure test setup – Photo" required>
       Photo the total static pressure test setup, existing condition
    </Photo>

    <Photo id="existing_static_pressure_read_out_photo" label="Existing condition total static pressure read out – Photo" required>
       Photo the total static pressure read out, existing condition
    </Photo>

    <p> Total duct leakage before air sealing:  <strong> { props.data.existing_duct_leakage_rate_radio ? props.data.existing_duct_leakage_rate_radio + ': ' : null } { props.data.existing_duct_leakage_rate_radio == 'CFM at 50Pa' ? props.data.duct_leakage_cfm50  : null } </strong></p>

    <Photo id="existing_area_of_significant_leakage_photo" label="Area of significant leakage – Photo" required>
        Photo of the duct leakage
    </Photo>

    <Photo id="existing_undersized_duct_or_return_photo" label="Undersized duct or return – Photo" required>
       Photo of the undersized duct or return
    </Photo>
   ...
   <Photo id="secure_and_seal_duct_photo" label="Secure and seal duct boots – Photo" required>
    ...
    <p>  Percentage of the duct system received additional insulation:  <strong> { props.data.postinstall_duct_insulation_percentage ? props.data.postinstall_duct_insulation_percentage : null } </strong> </p>
    ...
    <p> Total duct leakage after air sealing:  <strong> { props.data.postinstall_duct_leakage_rate ? props.data.postinstall_duct_leakage_rate : null } </strong> </p>
