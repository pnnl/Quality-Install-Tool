...
<ShowOrHide visible={props.data?.wall_air_leakage_rate === 'CFM at 50Pa'}>
    ...
    <NumberInput
        label="Conditioned Volume (ft3):"
        min={0}
        path="conditioned_volume_ft3"
    />
    |{' '}
    <LabelValue
        label="ACH50"
        decimalPlaces={1}
        type={
            !isNaN(props?.data?.air_leakage_cfm50) &&
            !isNaN(props?.data?.conditioned_volume_ft3)
                ? 'number'
                : 'string'
        }
        value={
            !isNaN(props?.data?.air_leakage_cfm50) &&
            !isNaN(props?.data?.conditioned_volume_ft3)
                ? (props?.data?.air_leakage_cfm50 * 60) /
                  props?.data?.conditioned_volume_ft3
                : 'N/A'
        }
    />
    <PhotoInput
        id="cfm50_photo"
        label="Starting blower door manometer showing the CFM50 value – Photo"
    >
        Take a photo of the manometer showing CFM50 of air leakage before air
        sealing and insulation work was performed.
    </PhotoInput>
    <PhotoInput
        id="conditioned_volume_photo"
        label="Conditioned floor area and air volume calculation – Photo"
    >
        Take a photo of the math used to determine the conditioned floor area
        and volume, label the rooms/sections and the corresponding length,
        width, and height.
    </PhotoInput>
</ShowOrHide>
    ...
    *	The ASHRAE 62.2 standard does not require the installation of a system smaller than 15 CFM (7 L/s)
    ...
    Use this section to document the existing condition of the exterior walls that need air sealing and insulation upgrades.

    <PhotoInput id="areas_needing_repair_photo" label="Areas that need repair/correction/improvements before insulation and air sealing – Photos">
      * Look for wet or dry rotted wood.
      * Look for mold.
      * Look for missing flashing around windows and doors, window and door trim, roof to wall transitions.
      * Look at trim pieces with missing or bad flashing that’s causing water to enter behind the siding.
      * Remember: air sealing and insulating a wall will dramatically accelerate the rate of rot and mold from missing flashing that is causing excessive wetting of the wall.
    </PhotoInput>
  ...
  <Tab eventKey="installation" title="Installation">
    ...
    *	Provide one representative photo.
    ...
    <PhotoInput id="through_wall_vent_penetration_photo" label="Through-wall vent penetration Air Sealed – Photo" >
    ...
    <PhotoInput id="insulation_product_photo" label="Insulation product installed – Photo" >
      Take a photo of the insulation product installed showing its R-value and/or R-value table.
    </PhotoInput>
...
<Radio
    label="What is the air sealing level you have achieved?"
    options={['Not Measured', 'CFM at 50Pa']}
    path="postinstall_air_leakage_level"
/>
<ShowOrHide
    visible={props.data?.postinstall_air_leakage_level === 'CFM at 50Pa'}
>
    <NumberInput
        label="CFM at 50Pa"
        min={0}
        path="postinstall_air_leakage_cfm50"
    />
    <PhotoInput
...
## Ventilation
 ...
 <Tab eventKey="report"  title="Report">
    ...
    ## Proposed Scope of Retrofit Work
    <Photo id="areas_needing_repair_photo" label="Areas that need repair/correction/improvements before insulation and air sealing – Photos" required>
      Photos of areas that need repair/correction/improvements before insulation and air sealing
    </Photo>
    ...
    <Photo id="through_wall_vent_penetration_photo" label="Through-wall vent penetration Air Sealed – Photo"  />
...
<Photo
    id="insulation_product_photo"
    label="Insulation product installed – Photo"
    required
>
    Photo of the insulation product installed showing its R-value and/or R-value
    table
</Photo>
