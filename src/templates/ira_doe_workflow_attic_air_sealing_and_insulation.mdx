...
<Tabs>
  ...
  <Tab eventKey="pre-installation" title="Pre-Installation">
    ...
    <ShowOrHide path="existing_insulation_type" value={['None']} matchCondition="Excludes">
      <Radio label="Will the existing insulation be reused?" options={['No', 'Yes, some', 'Yes, all']} path="existing_insulation_reused" />
    </ShowOrHide>
    ...
    *	The ASHRAE 62.2 standard does not require the installation of a system smaller than 15 CFM (7 L/s)
    ...
    Use this section to document the existing condition of the attic.
    ...
    <PhotoInput id="insulation_needed_photo" label="Area where insulation is needed – Photos">
      You can add many photos under this photo prompt. Take a photo of each proposed area that will get insulation. Document each knee walls, attic access door/scuttle panel, and all sections of attic.
    </PhotoInput>
  ...
  <ShowOrHide visible={props.data?.postinstall_attic_air_leakage_rate === 'CFM at 50Pa'}>
    ...
    <PhotoInput id="postinstall_cfm50_photo" label="Final blower door manometer showing the CFM50 value – Photo">
        Take a photo of the manometer showing CFM50 of air leakage after air sealing and insulation work was performed.
    </PhotoInput>
...
<ShowOrHide visible={props.data?.attic_air_leakage_rate == 'CFM at 50Pa'} >
    ...
    <ShowOrHide path="existing_insulation_type" value={['None']} matchCondition="Excludes">
      <p> Will the existing insulation be reused?  <strong> { props.data?.existing_insulation_reused} </strong></p>
    </ShowOrHide>
    ...
    <Photo id="attic_access_photo" label="Attic Access - Photo" required>
