<Tabs>
  ...
  <Tab eventKey="pre-installation" title="Pre-Installation">
    ...
    | <p><strong>ACH50: </strong>{String((props?.data?.air_leakage_cfm50 * 60)/props?.data?.conditioned_volume_ft3) }</p>
  ...
  <ShowOrHide visible={props.data?.postinstall_attic_air_leakage_rate === 'CFM at 50Pa'}>
    ...
    | <NumberInput label="Conditioned Volume (ft3):" min={0} path="postinstall_conditioned_volume_ft3" />
    | <p><strong>ACH50: </strong>{!isNaN(props?.data?.postinstall_air_leakage_cfm50) && !isNaN(props?.data?.postinstall_conditioned_volume_ft3) ? (props?.data?.postinstall_air_leakage_cfm50 * 60) / props?.data?.postinstall_conditioned_volume_ft3: 'N/A'}</p>
