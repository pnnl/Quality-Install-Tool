## Workflow: Heat Pump Water Heater Installation


<Tabs>
  <Tab eventKey="customer" style={{paddingTop:"1rem"}} title="Customer Info">
    ##### Builiding Location
    ...
    <StringInput label="Technician Name" path="technician_name" />
    <StringInput label="Installation Company" path="installation_company" />
    <DateInput label="Installation Date" path="install_date" />
    <StringInput label="Customer Name" path="customer_name" />
  </Tab>
  <Tab eventKey="planned" style={{paddingTop:"1rem"}} title="Planned Install Area">
    ...
    Length (ft) of the space that the HPWH is using to meet its air volume requirement.
    <NumberInput label="Length (ft)" min={0} path="volume_calculator.length"  />
    Width (ft) of the space that the HPWH is using to meet its air volume requirement.
    <NumberInput label="Width (ft)" path="volume_calculator.width"  />
    Height (ft) of the space that the HPWH is using to meet its air volume requirement.
  ...
  <Tab eventKey="sizing" style={{paddingTop:"1rem"}} title="HPWH Sizing">
  ...
  <Tab eventKey="install" style={{paddingTop:"1rem"}} title="Installation Day Photos">
  ...
  <Tab eventKey="report" style={{paddingTop:"1rem"}} title="Final Report">
    <PrintSection label="Print Report">
    ---
    **Heat Pump Water Heater Quality Installation Documentation Report**

    ##### Builiding Location
    **Street Address:** {props.doc.location?.street_address}
    **City:** {props.doc.location?.city}
    **State:** {props.doc.location?.state}
    **Zip:** {props.doc.location?.zip_code}


    **Technician Name:** {props.doc.technician_name}

    **Installation Company:** {props.doc.installation_company}

    **Date:** {props.doc.install_date}

    <Photo id="house_number_photo" label="House Number – Photo" required />

    Report for {props.doc.customer_name}, and energy efficiency program quality assurance and quality control implementation contractor staff.
    ...
    *The photos and information in this report use timestamps and geotags to prevent the reuse of photos from an unrelated project. The report's content is available only to the parties above. Any additional use such as for marketing and training purposes requires written permissions from the customer, in which case all timestamps, geotags, and other metadata from the photos and any identifiable characteristics that links the photos to this specific house will be removed prior. The homeowner / landlord is encouraged to retain a copy of this report which could help add value to a home sale and foster buyer confidence in the quality of the energy efficiency upgrade work that was performed. This report's data is protected by the standard data security and privacy best practices.*
    ...
    1. Picture (wide-angle) of the space where the HPWH is/will be installed
    2. Picture of the adjacent space if an adjacent space supplying air to meet the air volume requirement of the HPWH is used via a louvered door, high and low air grill, or ducting.
    3. Enter the length, width, and height measurements of the space used to meet the air volume requirement of the HPWH.
    4. Picture of the nameplate of the old water heater.
    ...
    6. A wide-angle picture of the completed HPWH installation
    7. Picture of the HPWH nameplate
    8. Picture of the HPWH screen showing the mode
    9. Picture of the utility energy management communication module CTA-2045 module if one is installed
    </Collapsible>
    ...
    **Electrical Availability:** {props.doc.electrical_availability}

    **Zip Code:** {props.doc.location?.zip_code}
    
    **Climate Zone:** {props.doc.location?.climate_zone}

    **Water heater will draw air from:** {props.doc.hpwh_location}

    **Number of Bedrooms:** {props.doc.bedrooms}

    **Number of Bathrooms:** {props.doc.bathrooms}

    **RESULT:** {props.doc.hpwh_voltage} Volt Heat Pump Water Heater with a {props.doc.hpwh_gallon_tank} Gallon Tank
