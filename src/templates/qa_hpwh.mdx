...
<Tabs>
  <Tab eventKey="project" style={{paddingTop:"1rem"}} title="Project">
    ...
    <StringInput label="Mailing Address" path="installer.mailing_address" />
    <StringInput label="Phone" path="installer.phone" />
    <StringInput label="Email" path="installer.email" />
...
| Air Volume = {props.data.volume_calculator?.length} x {props.data.volume_calculator?.width} x {props.data.volume_calculator?.height} = {props.data.volume_calculator?.length * props.data.volume_calculator?.width * props.data.volume_calculator?.height} ft<sup>3</sup> |
  ...
  <Tab eventKey="selection" style={{paddingTop:"1rem"}} title="Selection">
    <Collapsible header="HPWH Selection – Instructions">
      ...
      The heat pump can deliver the same heat into the water in the tank using 1/3 to 1/4 of
...
| Use | Avg Gallons of Hot Water per Usage | | Times Used During 1 Hour| | Gallons Used in 1 Hour|
| --- | --- | --- | --- | --- | --- |
| Shower | 20 | x | | = | |
| Shaving (.05 gal/min) | 2 | x | | = | |
| Hand dishwashing or food prep (2 gal/min) | 3 | x | | = | |
| Automatic dishwasher | 7 | x | | = | |
| Clothes washer - Top-loader | 25 | x | | = | |
| Clothes washer - H-Axis | 15 | x | | = | |
| | | | Total Peak Hour Demand | = | |

      *Source: [U.S. Department of Energy, Sizing a New Water Heater](https://www.energy.gov/energysaver/sizing-new-water-heater)*
  ...
  <Tab eventKey="report" style={{paddingTop:"1rem"}} title="Report">
    ...
    **Installation Date:** <Date date={props.data.install_date}/>
    ##### Building Location
