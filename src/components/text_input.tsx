 ...
 */
const TextInput: FC<TextInputProps> = ({id, label, updateValue, value, min, max, regexp}) => {
  ...
  return (
    <>
      <FloatingLabel className="mb-3" controlId={id} label={label}>
        <Form.Control as="textarea" onChange={event => handleChange(event.target.value)}
          placeholder="A placeholder" value={value || ""} isInvalid={Boolean(error)}/>
