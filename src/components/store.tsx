...
string,
Attachment | { blob: Blob, digest: string, metadata: Record<string, JSONValue> }
>
...
export const StoreProvider: FC<StoreProviderProps> = ({ children, dbName, docId }) => {
  ...
  async function processDBDocChange (db: PouchDB.Database, dbDoc: any) {
    ...
    if (db && dbDoc.hasOwnProperty('_attachments')) {
      ...
      for (const attachmentId in dbDocAttachments) {
        ...
        if ((digest != null) && (!attachments.hasOwnProperty(attachmentId) || attachments[attachmentId].digest !== digest)) {
          console.log('New attachment')
          const blobOrBuffer = await db.getAttachment(docId, attachmentId)

          if (blobOrBuffer instanceof Blob) {
            const blob = blobOrBuffer
            const metadata: Record<string, any> = blob.type === 'image/jpeg' ? singleAttachmentMetadata as Record<string, any> : {}
  ...
  useEffect(() => {
    ...
    (async function connectStoreToDB () {
      ...
      changesRef.current = db.changes({
        ...
        if ((change.doc != null) && change.doc._rev !== revisionRef.current) {
  ...
  const upsertDoc: UpsertDoc = (pathStr, data) => {
    ...
    if (db != null) {
      db.upsert(docId, function upsertFn (dbDoc: any) {
        ...
        if (pathStr.includes('metadata_')) { revisionRef.current = res.rev }
  ...
  return (
    <StoreContext.Provider value={{ attachments, data, metadata, upsertAttachment, upsertData, upsertDoc }}>
      {children}
...
function immutableUpsert (recipient: Objectish, path: NonEmptyArray<string>, target: any): Objectish {
  ...
  const newRecipient = isObject(recipient)
    ? Array.isArray(recipient) ? [...recipient] : { ...recipient } satisfies Record<string, any>
    : isNaN(parseInt(propName)) ? {} : []

  if (newPath.length === 0) {
    newRecipient[propName] = target
  } else {
    newRecipient[propName] = immutableUpsert(newRecipient[propName as keyof unknown], newPath as any, target)
  }
  return newRecipient
}
